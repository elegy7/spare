...
function makeModal(config) {
    if ($('.spare-modal').length == 0) {
        var $modal = $(
            `<div id="${config.id ||
                'modal'}" class="spare-modal modal fade" tabindex="-1" data-width="560" data-backdrop="static">` +
...
export function confirm(context, title, callback) {
    var $modal = makeModal({
        context,
        title,
        callback
...
export function show(context, title, callback) {
    var $modal = makeModal({
        context,
        title,
        callback
